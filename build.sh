...
cmake .. -DWITH_WEBRTC=ON  -DWEBRTC_ROOT_DIR=$webrtc_root_dir -DWEBRTC_INCLUDE_DIR=$webrtc_root_dir -DWEBRTC_LIBRARY_DEBUG=$webrtc_library_debug -DWEBRTC_LIBRARY_RELEASE=$webrtc_library_release -DWITH_FFMPEG=ON -DFFMPEG_ROOT_DIR=$ffmpeg_root_dir -DCMAKE_CXX_STANDARD=17 -DBUILD_SAMPLES=OFF -DBUILD_TESTS=OFF
